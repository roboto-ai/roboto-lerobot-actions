...
if [ "$QUIET" = true ]; then
    docker "${build_subcommand[@]}" --quiet -f $PACKAGE_ROOT/Dockerfile -t mcap_to_lerobot:latest $PACKAGE_ROOT &> /dev/null
else
    docker "${build_subcommand[@]}" -f $PACKAGE_ROOT/Dockerfile -t mcap_to_lerobot:latest $PACKAGE_ROOT
fi
