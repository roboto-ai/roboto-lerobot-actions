...
echo "Pushing mcap_to_lerobot:latest to Roboto's private registry"
image_push_args=(
...
image_push_args+=(mcap_to_lerobot:latest)
image_push_ret_code=0
image_uri=$($roboto_exe "${image_push_args[@]}")
image_push_ret_code=$?

if [ $image_push_ret_code -ne 0 ]; then
    echo "Failed to push mcap_to_lerobot:latest to Roboto's private registry"
    exit 1
fi

echo "Creating/updating mcap_to_lerobot action"
create_args=(
